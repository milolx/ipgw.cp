...
vlog = lib.vlog.Vlog("socket_util")
...
def make_unix_socket(style, nonblock, bind_path, connect_path):
    ...
    try:
        ...
        if bind_path is not None:
            ...
            lib.fatal_signal.add_file_to_unlink(bind_path)
            sock.bind(bind_path)
        ...
        if (bind_path is not None and
            ...
            lib.fatal_signal.unlink_file_now(bind_path)
        eno = lib.socket_util.get_exception_errno(e)
...
def check_connection_completion(sock):
    p = lib.poller.SelectPoll()
    p.register(sock, lib.poller.POLLOUT)
    pfds = p.poll(0)
    if len(pfds) == 1:
        revents = pfds[0][1]
        if revents & lib.poller.POLLERR:
            try:
