...
class Vlog:
    ...
    def init(log_file=None):
        ...
        lib.unixctl.command_register("vlog/reopen", "", 0, 0,
                                     Vlog._unixctl_vlog_reopen, None)
        lib.unixctl.command_register("vlog/set", "spec", 1, sys.maxint,
                                     Vlog._unixctl_vlog_set, None)
        lib.unixctl.command_register("vlog/list", "", 0, 0,
...
def add_args(parser):
    ...
    group.add_argument("-v", "--verbose", nargs="*",
                       help="Sets logging levels. Defaults to dbg.")
...
def handle_args(args):
    ...
    if log_file == "default":
        log_file = "%s/%s.log" % (lib.dirs.LOGDIR, lib.util.PROGRAM_NAME)
    ...
    for verbose in args.verbose:
        ...
        if msg:
            lib.util.ovs_fatal(0, "processing \"%s\": %s" % (verbose, msg))
