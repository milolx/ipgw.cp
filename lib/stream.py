...
class Stream(object):
    ...
    def open(name, dscp=DSCP_DEFAULT):
        ...
        if name.startswith("unix:"):
            suffix = lib.util.abs_file_name(lib.dirs.RUNDIR, suffix)
        error, sock = cls._open(suffix, dscp)
        if error:
            return error, None
        else:
            status = lib.socket_util.check_connection_completion(sock)
    ...
    def __scs_connecting(self):
        retval = lib.socket_util.check_connection_completion(self.socket)
    ...
    def recv(self, n):
        ...
        try:
            ...
            return (lib.socket_util.get_exception_errno(e), "")
    ...
    def wait(self, poller, wait):
        ...
        if wait == Stream.W_RECV:
            poller.fd_wait(self.socket, lib.poller.POLLIN)
        else:
            poller.fd_wait(self.socket, lib.poller.POLLOUT)
...
class PassiveStream(object):
    ...
    def open(name):
        ...
        if name.startswith("punix:"):
            bind_path = lib.util.abs_file_name(lib.dirs.RUNDIR, bind_path)
        error, sock = lib.socket_util.make_unix_socket(socket.SOCK_STREAM,
                                                       True, bind_path, None)
        if error:
            return error, None
    ...
    def close(self):
        ...
        if self.bind_path is not None:
            lib.fatal_signal.unlink_file_now(self.bind_path)
            self.bind_path = None
    ...
    def accept(self):
        ...
        while True:
            try:
                ...
                lib.socket_util.set_nonblocking(sock)
                return 0, Stream(sock, "unix:%s" % addr, 0)
            except socket.error, e:
                error = lib.socket_util.get_exception_errno(e)
                if error != errno.EAGAIN:
                    # XXX rate-limit
...
class UnixStream(Stream):
    ...
    def _open(suffix, dscp):
        connect_path = suffix
        return  lib.socket_util.make_unix_socket(socket.SOCK_STREAM,
...
class TCPStream(Stream):
    @staticmethod
    def _open(suffix, dscp):
        error, sock = lib.socket_util.inet_open_active(socket.SOCK_STREAM,
                                                       suffix, 0, dscp)
        if not error:
