...
import lib.dirs
import lib.fatal_signal
#import lib.lockfile
import lib.process
import lib.socket_util
import lib.timeval
import lib.util
import lib.vlog

vlog = lib.vlog.Vlog("daemon")
...
def make_pidfile_name(name):
    ...
    if name is None or name == "":
        return "%s/%s.pid" % (lib.dirs.RUNDIR, lib.util.PROGRAM_NAME)
    else:
        return lib.util.abs_file_name(lib.dirs.RUNDIR, name)
...
def set_pidfile(name):
    ...
    Otherwise, it is taken relative to lib.util.RUNDIR, which is
    $(prefix)/var/run by default.

    If 'name' is null, then lib.util.PROGRAM_NAME followed by ".pid" is
    used."""
    global _pidfile
...
def _make_pidfile():
    ...
    lib.fatal_signal.add_file_to_unlink(tmpfile)
    try:
    ...
    lib.fatal_signal.add_file_to_unlink(_pidfile)

    # Delete the temporary pidfile if it still exists.
    if not _overwrite_pidfile:
        error = lib.fatal_signal.unlink_file_now(tmpfile)
        if error:
...
def _fork_and_wait_for_startup():
    ...
    if pid > 0:
        ...
        # Running in parent process.
        os.close(rfd)
        lib.timeval.postfork()
        #lib.lockfile.postfork()
...
def _fork_notify_startup(fd):
    if fd is not None:
        error, bytes_written = lib.socket_util.write_fully(fd, "0")
        if error:
    ...
    while True:
        ...
        if retval < 0:
            ...
            status_msg = ("pid %d died, %s"
                          % (daemon_pid, lib.process.status_msg(status)))
            ...
            if _should_restart(status):
                ...
                if (last_restart is not None and
                    lib.timeval.msec() < last_restart + 10000):
...
def _close_standard_fds():
    ...
    null_fd = lib.socket_util.get_null_fd()
    if null_fd >= 0:
        os.dup2(null_fd, 0)
        os.dup2(null_fd, 1)
        os.dup2(null_fd, 2)
...
""" % (lib.dirs.RUNDIR, lib.util.PROGRAM_NAME))
...
def add_args(parser):
    ...
    group.add_argument("--monitor", action="store_true",
            help="Monitor %s process." % lib.util.PROGRAM_NAME)
