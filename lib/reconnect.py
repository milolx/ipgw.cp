...
class Reconnect(object):
    ...
    value of lib.time.msec().  (Perhaps this design should be revisited
    later.)"""
    ...
    def get_probe_interval(self):
        ...
        self.activity() being called, self.run() returns lib.reconnect.PROBE.
        If the interval passes again without self.activity() being called,
        self.run() returns lib.reconnect.DISCONNECT."""
    ...
    def set_probe_interval(self, probe_interval):
        ...
        lib.reconnect.PROBE.  If the interval passes again without
        self.activity() being called, self.run() returns
        lib.reconnect.DISCONNECT.
    ...
    def force_reconnect(self, now):
        ...
        connect), forces self.run() to return lib.reconnect.DISCONNECT the next
    ...
    def connecting(self, now):
        ...
        attempt will be aborted (by returning lib.reconnect.DISCONNECT from
        self.run())."""
    ...
    def listening(self, now):
        ...
        return lib.reconnect.CONNECT from self.run() to tell the client to try
        listening again."""
    ...
    def listen_error(self, now, error):
        ...
        will back off and eventually return lib.reconnect.CONNECT from
    ...
    def connected(self, now):
        ...
        returning lib.reconnect.PROBE from self.run().  If the timer expires
        again without being reset, the connection will be aborted (by returning
        lib.reconnect.DISCONNECT from self.run()."""
    ...
    def run(self, now):
        ...
        value is one of:
            ...
            - Active client, lib.reconnect.CONNECT: The client should start a
            ...
            - Passive client, lib.reconnect.CONNECT: The client should try to
            ...
            - lib.reconnect.DISCONNECT: The client should abort the current
              connection or connection attempt or listen attempt and call
              self.disconnected() or self.connect_failed() to indicate it.

            - lib.reconnect.PROBE: The client should send some kind of request
