...
Message = lib.jsonrpc.Message
vlog = lib.vlog.Vlog("unixctl_server")
strtypes = types.StringTypes


class UnixctlConnection(object):
    def __init__(self, rpc):
        assert isinstance(rpc, lib.jsonrpc.Connection)
        self._rpc = rpc
    ...
    def _process_command(self, request):
        assert isinstance(request, lib.jsonrpc.Message)
        assert request.type == lib.jsonrpc.Message.T_REQUEST
        ...
        command = lib.unixctl.commands.get(method)
        if command is None:
...
def _unixctl_version(conn, unused_argv, version):
    ...
    version = "%s (Open vSwitch) %s" % (lib.util.PROGRAM_NAME, version)
    conn.reply(version)

class UnixctlServer(object):
    def __init__(self, listener):
        assert isinstance(listener, lib.stream.PassiveStream)
    ...
    def run(self):
        for _ in range(10):
            ...
            if not error:
                rpc = lib.jsonrpc.Connection(stream)
    ...
    def create(path, version=None):
        ...
        version command.  If None, lib.version.VERSION is used."""

        assert path is None or isinstance(path, strtypes)

        if path is not None:
            path = "punix:%s" % lib.util.abs_file_name(lib.dirs.RUNDIR, path)
        else:
            path = "punix:%s/%s.%d.ctl" % (lib.dirs.RUNDIR,
                                           lib.util.PROGRAM_NAME, os.getpid())

        if version is None:
            version = lib.version.VERSION

        error, listener = lib.stream.PassiveStream.open(path)
        if error:
            lib.util.ovs_error(error, "could not initialize control socket %s"
                               % path)
            return error, None

        lib.unixctl.command_register("version", "", 0, 0, _unixctl_version,
                                     version)
...
class UnixctlClient(object):
    def __init__(self, conn):
        assert isinstance(conn, lib.jsonrpc.Connection)
        self._conn = conn
    ...
    def create(path):
        ...
        unix = "unix:%s" % lib.util.abs_file_name(lib.dirs.RUNDIR, path)
        error, stream = lib.stream.Stream.open_block(
            lib.stream.Stream.open(unix))

        if error:
            vlog.warn("failed to connect to %s" % path)
            return error, None

        return 0, UnixctlClient(lib.jsonrpc.Connection(stream))
