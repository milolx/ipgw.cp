...
class Message(object):
    ...
    def __str__(self):
        ...
        if self.params is not None:
            s.append("params=" + lib.json.to_string(self.params))
        if self.result is not None:
            s.append("result=" + lib.json.to_string(self.result))
        if self.error is not None:
            s.append("error=" + lib.json.to_string(self.error))
        if self.id is not None:
            s.append("id=" + lib.json.to_string(self.id))
        return ", ".join(s)
...
class Connection(object):
    ...
    def send(self, msg):
        ...
        self.output += lib.json.to_string(msg.to_json())
        if was_empty:
            self.run()
        return self.status
...
class Session(object):
    ...
    def open(name):
        ...
        'name', which should be a string acceptable to lib.stream.Stream or
        lib.stream.PassiveStream's initializer.
        ...
        to be dropped."""
        reconnect = lib.reconnect.Reconnect(lib.timeval.msec())
        reconnect.set_name(name)
        reconnect.enable(lib.timeval.msec())

        if lib.stream.PassiveStream.is_valid_name(name):
            reconnect.set_passive(True, lib.timeval.msec())

        if lib.stream.stream_or_pstream_needs_probes(name):
    ...
    def open_unreliably(jsonrpc):
        ...
        reconnect.connected(lib.timeval.msec())
    ...
    def __connect(self):
        ...
        if not self.reconnect.is_passive():
            error, self.stream = lib.stream.Stream.open(name)
            if not error:
                self.reconnect.connecting(lib.timeval.msec())
            else:
                self.reconnect.connect_failed(lib.timeval.msec(), error)
        elif self.pstream is not None:
            error, self.pstream = lib.stream.PassiveStream.open(name)
            if not error:
                self.reconnect.listening(lib.timeval.msec())
            else:
                self.reconnect.connect_failed(lib.timeval.msec(), error)
    ...
    def run(self):
        if self.pstream is not None:
            ...
            if error == 0:
                if self.rpc or self.stream:
                    ...
                    self.__disconnect()
                self.reconnect.connected(lib.timeval.msec())
                self.rpc = Connection(stream)
            elif error != errno.EAGAIN:
                self.reconnect.listen_error(lib.timeval.msec(), error)
                self.pstream.close()
                self.pstream = None
        ...
        if self.rpc:
            ...
            if self.rpc.get_backlog() < backlog:
                ...
                self.reconnect.activity(lib.timeval.msec())

            error = self.rpc.get_status()
            if error != 0:
                self.reconnect.disconnected(lib.timeval.msec(), error)
                self.__disconnect()
        elif self.stream is not None:
            self.stream.run()
        ...
        action = self.reconnect.run(lib.timeval.msec())
        if action == lib.reconnect.CONNECT:
            self.__connect()
        elif action == lib.reconnect.DISCONNECT:
            self.reconnect.disconnected(lib.timeval.msec(), 0)
            self.__disconnect()
        elif action == lib.reconnect.PROBE:
            if self.rpc:
    ...
    def wait(self, poller):
        ...
        self.reconnect.wait(poller, lib.timeval.msec())
    ...
    def force_reconnect(self):
        self.reconnect.force_reconnect(lib.timeval.msec())
